...
export const pushJob = async (req, res) => {
    ...
    await client.rPush(`queue:${topic}`, JSON.stringify({ jobId, createdAt, updatedAt: createdAt, ...job }));

    res.status(200).json({ jobId, message: 'Job added successfully' });
};

// Get Job from Topic
export const getJobToRun = async (req, res) => {
    ...
    const rawJob = await client.blPop(`queue:${topic}`, 60);
    ...
    if (rawJob.element) {
        ...
        res.status(200).json({ job, message: 'Job to run' });
    } else {
        return res.status(404).json({ message: 'No job available' });
    }
};
...
export const markCompleted = async (req, res) => {
    ...
    if (jobToRemove) {
        ...
        res.status(200).json({ message: 'Job completed successfully', job: jobToRemove });
    } else {
...
export const retryExpiredJobs = async () => {
    ...
    for (const topic of topics) {
        ...
        for (const job of jobs) {
            ...
            console.log("JOB ID IS ", jobId, "Updated IS", updatedAt, "CreatedAt IS", createdAt, "Now IS", now, "DIFFERENCE IS", (now - updatedAt), "SHOULD BE MOVED? ", (now - updatedAt) > 60000);
